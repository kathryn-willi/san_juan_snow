---
title: "Web Scraping"
author: "Matthew Ross, Homework Submission by Katie Willi"
date: "10/20/2019"
...
splits <- str_split_fixed(links,'/',5)

dataset2 <- splits[,5]

filez <- paste0('data/',dataset2)

 for(i in 1:length(filez)){
   download.file(links[i],destfile=filez[i])
 }
...
datas <- read_delim('data/Serially-Complete-SBSP-text07.txt', col_names=headers,  delim=" ")

q_binder <- function(dataset = datas,
                     name = "Senator Beck"){
  df <- mutate(dataset, site = name)
  return(df)
}

q_binder(dataset = datas,
         name = "Senator Beck") %>%
  slice(1)
...
our_forcing_reader <- function(filez){
  name = str_split_fixed(filez,'-',3)[,3]
  df <- read_delim(filez, col_names=headers,  delim=" ") %>%
    mutate(site = name)
}
